...
struct _cache_t
{
	...
	blk_t victim_buffer_blks[VICTIMBUFFERSIZE];
	char victim_buffer [VICTIMBUFFERSIZE][BlockSize];
	unsigned int victim_buffer_evict = 0;
...
heapMgtInit(void){
	...
	fprintf(_reportFile,
			 ...
			 \n         { assoc = %ld , num_sets = %ld , num_blks = %ld }\
...
fetch_ftn(char  *spmAddr_, char *memAddr_){
		...
		memcpy ( spmAddr_  , memAddr_, BlockSize   );
		return BlockSize;
	}
...
eviction_ftn(char  *spmAddr_, char *memAddr_ ){
	...
	memcpy ( memAddr_  , spmAddr_ , BlockSize );
	return BlockSize;	
}
...
addrTranslation (unsigned int *gaddr__, int opcode){ //, int datatype , int datasize){
	...
	if (!blk) // Cache miss and victim buffer miss, fetch from main memory
	{
		...
		blk_t * evict_blk = &victim_buffer_blks[victim_buffer_evict];
		...
		if (blk->status != INVALID)
		{
			...
			victim_buffer_evict = (victim_buffer_evict +1) % VICTIMBUFFERSIZE ;
		}
